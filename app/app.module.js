...
app.controller('weatherapiController', function($scope, $http) {
   /* $scope.placeid = 1;
    ...
    $scope.citytemp = "";
   // $http.get("https://jsonplaceholder.typicode.com/todos/"+$scope.placeid).success(function(response) {$scope.placeData= response;});
 ...
 /*$scope.updatePlaceData = function (placeid) {
        ...
        $scope.placeData = $http.get("https://jsonplaceholder.typicode.com/todos/"+placeid).success(function(response) {$scope.placeData= response;
